...
#### How session work maps to the SRC DMP template

Use the template’s **own question wording** (below) so students can paste into the matching field in Word/DMPonline. Full student handout: [`assignment/session-to-dmp-map.md`](../assignment/session-to-dmp-map.md).

| SRC template question (verbatim) | What students produce in these sessions | Directly usable in the DMP? |
|----------------------------------|-------------------------------------------|----------------------------|
| **General Information** (title, leader, date, version) | Project title, people, dates in README | Yes — copy into header fields |
| *How will data be collected, created or reused?* | Methods note in README; decision log on reuse vs new data | Yes — draft for collection/reuse and provenance |
| *What types of data will be created and/or collected, in terms of data format and amount/volume of data?* | Folder tree; file-type inventory; naming conventions | Yes — formats, organisation, approximate volume |
| *How will the material be documented and described, with associated metadata relating to structure, standards and format for descriptions of the content, collection method, etc.?* | README, `DECISIONS.md`, data-dictionary stub, folder conventions | **Strong fit** — organisation, versioning, accompanying docs |
| *How will data quality be safeguarded and documented (for example repeated measurements, validation of data input, etc.)?* | Notes on checks, validation, peer review of docs | Partial — quality *of data handling/docs*; collection QC mainly in the protocol |
| *How is storage and backup of data and metadata safeguarded during the research process?* | Storage location note; raw vs derived separation; backup habits | Yes — during-project storage/backup narrative |
| *How is data security and controlled access to data safeguarded, in relation to the handling of sensitive data and personal data, for example?* | What must *not* go in public repos or AI prompts; access notes | Yes — especially personal/sensitive data handling |
| *How is data handling according to legal requirements safeguarded, e.g. in terms of handling of personal data, confidentiality and intellectual property rights?* / *How is correct data handling according to ethical aspects safeguarded?* | Sharing limits; GDPR-aware AI use; consent/location of ethics docs | Supporting drafts — students still check with KI support functions |
| *How, when and where will research data or information about data (metadata) be made accessible? Are there any conditions, embargoes and limitations on the access to and reuse of data to be considered?* / *In what way is long-term storage safeguarded, and by whom? How will the selection of data for long-term storage be made?* | 5-year interpretability; what could be shared later vs metadata-only | Supporting drafts — repository choice may come later |
| *Who is responsible for data management and (possibly) supports the work with this while the research project is in progress? Who is responsible for data management, ongoing management and long-term storage after the research project has ended?* / *What resources (costs, labour input or other) will be required for data management (including storage, back-up, provision of access and processing for long-term storage)? What resources will be needed to ensure that data fulfil the FAIR principles?* | Roles in README (who owns folder, docs, DMP updates); rough time/cost note | Yes — roles and resources during/after the project |

#### Design implication (make sessions assignment-ready)

Every major activity should end with an explicit **“paste into your DMP”** step:

1. Build or refine the **project package** (folder + docs).
2. Open the **SRC template** (or DMPonline equivalent).
3. Transfer today’s text into the matching **template question(s)**, marked as *draft for assignment*.

Provide a one-page handout: **“Session → SRC DMP section map”** ([`assignment/session-to-dmp-map.md`](../assignment/session-to-dmp-map.md)) using the template’s verbatim questions. Day 2 submission = project package **plus** a partially completed DMP (at least answers under the documentation, data description, and storage questions).
...
2. Skim the [SRC DMP template](../Swedish_Research_Council_Template__v5.docx) headings + Session → DMP map handout.
...
- [ ] Put SRC template + Session → DMP map handout in Canvas
