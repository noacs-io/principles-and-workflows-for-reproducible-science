...
- [ ] **#9** Run sheets for the four blocks: every activity names its principle, versioning is taught generically rather than as a Git lesson, and each major activity ends with an explicit **“paste into SRC DMP”** step
- [ ] **#10** Canvas module: both self-study morning packs (required vs optional), SRC template + **Session → SRC DMP map**, the student note on project package → DMP versus protocol, and the submission place — *drafts in [`canvas/`](../canvas/); publishing to the course room still required; Good Enough Practices split into Part A / Part B (D021); other readings still to be fixed*
- [ ] **#11** Materials pack: one good + one poor synthetic project folder [Johanna], README / decision-log / data-dictionary templates, and the **5-year rubric**
...
- [ ] **#15** AI package: student rules, prompts for the three AI tasks in the brief, a demo that includes one wrong answer, and the disclosure template
...
Session ILOs need no separate freeze — the five locked outcomes are in the brief.
