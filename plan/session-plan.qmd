...
flowchart LR
  ...
  E --> F[Within-team peer critique]
```

| TBL element | Day 1 focus | Day 2 focus |
|-------------|-------------|-------------|
| Pre-work | Definition + caveats; KI rules; **AI rules (Copilot-first)**; read GEP **Introduction**, **Data Management** 1–6, **Project Organization**, **Collaboration** 1; FAIR videos | Read GEP **Keeping Track of Changes**, **Data Management** 7, **Collaboration** 2–5 (versioning + sharing); prepare own package for afternoon apps |
...
| Application | **App 1:** unlabelled folder critique against the **5-year rubric** → short findings on a shared board. **App 2:** own folder + README; paste **data description** and **documentation**; within-team peer critique (each presents, then discuss) | **App 1:** alone → team → board: choose history + sharing; paste **storage** / **security**; post **tensions / unclear points**. **App 2:** own package + AI docs; paste **data quality** / **who is responsible**; within-team peer critique |
...
3. Application 1: compare two synthetic project folders (unlabelled) against the **5-year rubric** — each team posts short findings to a shared board (hybrid-friendly; name folder + criterion); facilitator samples contrasts.
...
5. Within-team peer critique: each person presents their package; teammates discuss against the 5-year rubric (clarity, longevity, ethics, findability).
...
2. Application 1: alone → team → board (D044) — choose readable history + sharing/access limits for *your* project; paste **storage** and **security**; team posts **2–3 tensions or unclear points**; facilitator samples.
3. Application 2: refine folder + README / decision log (manual or Copilot); paste **data quality** and **who is responsible**.
4. Within-team peer critique against the **5-year rubric** *and* “Could a teammate find matching text in your DMP draft?” (each person presents; then discuss).
...
Student-facing copy: [`canvas/pages/five-year-rubric.md`](../canvas/pages/five-year-rubric.md). Used from Day 1 App 1 onward (D043).
