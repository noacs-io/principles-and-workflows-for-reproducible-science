...
1. Orientation: Turing Way definition + caveats; KI passages on *reproducera* / reproducibility; **course DMP assignment**; schedule; AI rules.
...
3. Read: **Good Enough Practices, Part A** and the KI guidelines named in the brief ([Riktlinjer för forskning](https://medarbetare.ki.se/media/99275/download) §§6–7; documentation and data management guidelines).
4. Explore: one *good* and one *poor* synthetic project folder (screenshots or zip) — which choices support reproducibility or auditability.
...
4. Application 2: design folder + README for a shared global-health vignette; say how each choice supports the definition (or auditability); paste **data description** (both SRC prompts) and **documentation** into the DMP.
5. Inter-team critique with rubric (clarity, longevity, ethics, findability).
6. Exit ticket: one change for your real project tomorrow + how it supports reproducibility or auditability.
...
- [ ] **#10** Canvas module: both self-study morning packs (required vs optional), SRC template + **Session → SRC DMP map** (six areas, D023), the student note on project package → DMP versus protocol, and the course assignment place — *drafts in [`canvas/`](../canvas/); publishing to the course room still required; pre-readings named in the brief (now includes Riktlinjer för forskning §§6–7); FAIR URL and the data-sharing document still to be fixed*
