...
|-------------|-------------|-------------|
| Pre-work | Why structure & documentation matter; KI rules; read **Good Enough Practices Part A**; inspect example repos | Read **Good Enough Practices Part B**; versioning (dated log or Git); AI-assisted drafting norms; prepare own project materials |
...
| Application | Teams design a folder + README for a shared vignette; paste **data description** and **documentation** into the DMP | Teams work on *their* PhD project package; paste **data quality**, **storage**, **security**, and **who is responsible**; peer review against the 5-year rubric |
...
2. Skim the [SRC DMP template](../Swedish_Research_Council_Template__v5.docx) headings + the **Session → SRC DMP map** in this plan (the six areas in the brief).
3. Read: **Good Enough Practices, Part A** and the KI guidelines (named in the brief).
...
3. Application 1: with Copilot (or approved alternative), draft a folder tree for *your* PhD project; save prompt + output; present to the team.
4. Application 2: design folder + README for a shared global-health vignette; paste **data description** (both SRC prompts) and **documentation** into the DMP.
5. Inter-team critique with rubric (clarity, longevity, ethics, findability).
6. Exit ticket: one change for your real project tomorrow + which DMP section it will feed.
...
1. Read: **Good Enough Practices, Part B** and the Day 2 readings named in the brief — change tracking, sharing and DOIs, licensing and citation.
2. Mini-lesson: versioning so the project passes the 5-year test — dated copies and a decision log, or Git (D002 / D014); neither is the fallback.
3. AI task: turn your notes into a README + `DECISIONS.md` (or similar); verify and edit.
4. **Paste/adapt** today’s materials into the SRC template under **data quality**, **storage**, **security**, and **who is responsible** — mark as *draft*.
...
2. Application: refine the live package (folder + docs); paste **data quality**, **storage**, **security**, and **who is responsible**; optional first GitHub commit for those ready.
...
5. Close with a take-away checklist: project package + **partial SRC DMP** + AI-use disclosure, for the course assignment (no session hand-in).
...
- [ ] **#10** Canvas module: both self-study morning packs (required vs optional), SRC template + **Session → SRC DMP map** (six areas, D023), the student note on project package → DMP versus protocol, and the course assignment place — *drafts in [`canvas/`](../canvas/); publishing to the course room still required; pre-readings named in the brief; FAIR URL and the data-sharing document still to be fixed*
