...
9. **First-run TBL delivery:** prefer low friction for readiness tests (InteDashboard if available, else Canvas quizzes). Application exercises use shared-board share-outs and **within-team** peer critique (each person presents; teammates discuss) when packages differ or the room is hybrid (D030 / D041) — not classic simultaneous report. Do not block on new hosting unless already running.
